...
| `IMPROVE` / `IMPROVE_INTERVAL_S` / `IMPROVE_MAX_TRIES` | Article XI self-improvement cycle: on by default every hour, trying up to 3 mechanic-proposed patches per cycle against the project's own suites in a scratch copy; `IMPROVE=0` disables. `IMPROVE_SUITES` limits which suites are the oracle (comma list of governor, work, settlement, mechanic) |
