...
async function load(){
  ...
  else{const hh=g.health||{};$('gw').innerHTML=[['gateway',hh.ok?`<span class=good>up</span> · build ${esc(hh.gateway_build)} · up ${hh.uptime_s>=3600?(hh.uptime_s/3600).toFixed(1)+' h':hh.uptime_s+' s'}`:'<span class=bad>reports not ok</span>'],['server',g.version?'Ollama '+esc(g.version):'—'],['loaded now',(g.loaded||[]).length?g.loaded.map(m=>`<b>${esc(m.model)}</b> · ${m.parameter_size||''} ${m.quantization||''} · ${m.size_gb} GB on disk · ${m.vram_gb} GB in VRAM · until ${esc(m.expires_at||'')}`).join('<br>'):'nothing loaded — the next call pays the load'],['served',(g.available||[]).map(esc).join(', ')||'—']]
